...
export function TopScorers({ entries }: Props) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: 1, color: '#a78bfa', textTransform: 'uppercase', marginBottom: 6 }}>
      ...
      {top.map((e) => (
        <div key={e.id} style={{ fontSize: 14, color: '#e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontWeight: 700, color: '#a78bfa' }}>+{e.pointsToday} pts</span>
        </div>
      ))}
    </div>
  );
